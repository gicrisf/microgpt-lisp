...
(defun save-checkpoint (state-table m v step path)
  "Save full training checkpoint: weights, Adam buffers, and step count."
  (with-open-file (out path :direction :output :if-exists :supersede)
    (format out "step ~a~%" step)
    (format out "adam-m ~{~f~^ ~}~%" (coerce m 'list))
    (format out "adam-v ~{~f~^ ~}~%" (coerce v 'list))
...
(defun load-checkpoint (path)
  "Load full checkpoint. Returns (values state-table m v step)."
  (with-open-file (in path :direction :input)
    (let* ((step-line (read-line in))
           (step (parse-integer (second (uiop:split-string step-line :separator " "))))
           (m-line (read-line in))
           (m-vals (mapcar (lambda (s) (read-from-string s))
                           (rest (uiop:split-string m-line :separator " "))))
           (v-line (read-line in))
           (v-vals (mapcar (lambda (s) (read-from-string s))
                           (rest (uiop:split-string v-line :separator " "))))
           (m (make-array (length m-vals) :initial-contents m-vals))
           (v (make-array (length v-vals) :initial-contents v-vals))
...
(defun init-state-table (vocab-size n-embd block-size n-layer)
  "Create a fresh state-table with random weights."
...
(defun flatten-params (state-table)
  "Flatten all Value objects in state-table into a single vector."
...
(defun train (names uchars bos vocab-size n-embd n-head n-layer block-size head-dim
              &key (num-steps 100000) (learning-rate 0.01) (save-path nil) (checkpoint-path nil))
  (multiple-value-bind (loaded-st loaded-m loaded-v start-step)
      (if checkpoint-path
          (progn
            (format t "Resuming from checkpoint ~a~%" checkpoint-path)
            (load-checkpoint checkpoint-path))
          (values (init-state-table vocab-size n-embd block-size n-layer) nil nil 0))
    (let* ((state-table loaded-st)
           (params (flatten-params state-table))
           (num-params (length params))
           (beta1 0.85)
           (beta2 0.99)
           (eps-adam 1e-8)
           (m (or loaded-m (make-array num-params :initial-element 0.0)))
           (v (or loaded-v (make-array num-params :initial-element 0.0))))
      (format t "Num params: ~a~%" num-params)
      (loop for step from start-step below num-steps
            do (let* ((doc (aref names (mod step (length names))))
      ...
      (when save-path
        (save-checkpoint state-table m v num-steps save-path)
        (format t "Saved checkpoint to ~a~%" save-path))
      state-table)))
...
(defun main (&key checkpoint-path)
  (let* ((names (let ((n (shuffle (load-names))))
         ...
         (save-path (asdf:system-relative-pathname :microgpt "model.ckpt"))
